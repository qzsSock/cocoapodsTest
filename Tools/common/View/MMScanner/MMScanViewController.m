...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (metadataObjects.count>0) {
        ...
        if([NSString isNULLString:metadataObject.stringValue])
        {
            [self showError:@"图片中未识别到二维码数据"];
            return;
        }
...
- (void)drawTitle
{
    if (!_tipTitle)
    {
        ...
        _tipTitle.center = CGPointMake(CGRectGetWidth(self.view.frame)/2, self.view.center.y + self.view.frame.size.width/2 - 35 - 50);
...
- (void)changeScanCodeType:(MMScanType)type {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        _tipTitle.text = type == MMScanTypeQrCode ? @"对准二维码,即可自动扫描" : @"对准条码,即可自动扫描";
